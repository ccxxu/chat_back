    ...
    <body class="white-bg">
        <div class="wrapper wrapper-content animated fadeInRight ibox-content">
            <form class="form-horizontal m" id="form-role-edit">
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label">云课分类</label>
                    <div class="col-sm-8">
                		<select class="form-control" name="pid">
                			<c:forEach items="${cate }" var="item" >
							<option value="${item._id }" <c:if test="${model.pid == item._id }">selected</c:if>>${item.name }</option>
							</c:forEach>
						</select>
					</div>
                </c:if>
