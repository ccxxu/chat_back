	...
	<body class="white-bg">
		<div class="wrapper wrapper-content animated fadeInRight ibox-content">
			<form class="form-horizontal m" id="form-group-edit">
				<input id="id" name="id" type="hidden" value="${gm.id}"/>
				<div class="form-group">
					<label class="col-sm-3 control-label is-required">用户组名称：</label>
					<div class="col-sm-8">
						<input class="form-control" type="text" name="name" id="name" value="${gm.name }" required>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-3 control-label is-required">用户组描述：</label>
					<div class="col-sm-8">
						<input class="form-control" type="text" name="descc" id="descc" value="${gm.descc }" required>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-3 control-label is-required">显示排序：</label>
					<div class="col-sm-8">
						<input class="form-control" type="text" name="orderNum" value="${gm.orderNum }" required>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-8">
					    <label class="radio-box" >
							<input type="radio" name="status" value="1" <c:if test="${gm.status != '0' }">checked</c:if> />
							是
						</label>
						<label class="radio-box" >
							<input type="radio" name="status" value="0" <c:if test="${gm.status == '0' }">checked</c:if> />
							否
						</label>
					</div>
				</div>
	 	...
	 	<script>
	        ...
	        function submitHandler() {
		        if ($.validate.form()) {
		            $.operate.save("${ctx}/system/group/save", $('#form-group-edit').serialize());
		        }
		    }
